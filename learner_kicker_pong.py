...
from kicker_pong.model_value_estimator import ValueEstimator
...
def main():
    ...
    s_path_model = "/home/prock/models/reinforce_with_baseline_kicker_pong_v2_"
    s_path_reward = "/home/prock/data/reinforce_with_baseline_reward_train_data_v2.txt"
    s_path_batch_loss = "/home/prock/data/reinforce_with_baseline_batch_loss_train_data_v2.txt"
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as session:
        ...
        value_predictor = ValueEstimator(session=session, state_size=state_size, hidden_size_1=300, hidden_size_2=600,
                                         hidden_size_3=600, hidden_size_4=600, hidden_size_5=300, learning_rate=1e-5)
        ...
        try:
                ...
                for j in range(max_episode_length):
                    ...
                    if terminal:
                        ...
                        episode_history.learning_value_pg = learning_advantage(episode_history.discounted_returns,
                                                                               episode_history.estimated_value)
            ...
            with open(s_path_reward, "w") as fp:
                fp.writelines('Trainingsdaten kicker_pong mit REINFORCE with basline Algorithmus '
                              '(Belohnung pro Episode) ' + explore_exploit_setting + '\n')
                for k in episode_rewards:
                    fp.write("%s\n" % k)
            with open(s_path_batch_loss, "w") as fp:
                fp.writelines('Trainingsdaten kicker_pong mit REINFORCE with basline Algorithmus (Kosten pro Episode) '
